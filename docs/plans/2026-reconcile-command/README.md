...
- Keep source-specific logic in adapter-like packages, starting with `adapters/nextcloudmemories`
- Do not force reconciliation into the existing `adapters.Reader` / `adapters.Runner` upload abstraction
...
This command becomes the natural home for the user reconciliation mode already drafted in:
